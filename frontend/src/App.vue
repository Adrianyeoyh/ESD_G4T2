...
const isConsultationReviewPage = computed(
...
const normalizeConsultationDrug = (drug, idx) => ({
  ...
  quantity: Math.max(
    0,
    Number(drug.quantity ?? drug.stock ?? drug.Quantity ?? 0),
  ),
...
const consultationFilteredDrugs = computed(() => {
  const search = String(consultationDrugSearch.value || "")
    .trim()
    .toLowerCase();
  if (!search) {
  ...
  consultationDrugCatalogue.value
    .map((drug) => ({
      ...
      selectedQuantity: Math.max(
        0,
        Number(consultationDrugSelections.value[drug.id] ?? 0),
      ),
    }))
...
const consultationSelectedDrugCount = computed(() =>
  consultationSelectedDrugs.value.reduce(
    (total, drug) => total + drug.selectedQuantity,
    0,
  ),
);
...
const syncConsultationDrugSelections = () => {
  ...
  for (const drug of consultationDrugCatalogue.value) {
    const currentQuantity = Number(
      consultationDrugSelections.value[drug.id] ?? 0,
    );
...
const setConsultationDrugQuantity = (drugId, rawValue) => {
  const drug = consultationDrugCatalogue.value.find(
    (item) => item.id === Number(drugId),
  );
  if (!drug) {
    return;
  }
...
const composeConsultationVisitNotes = (visitNotes, drugs) => {
  ...
  const drugSummary = drugs
    .map(
      (drug) =>
        `- ${drug.name} x${drug.selectedQuantity} (available ${drug.quantity})`,
    )
    .join("\n");

  return [trimmedNotes, "Prescribed Drugs:", drugSummary]
    .filter(Boolean)
    .join("\n\n");
};
...
const normalizedRecords = computed(() =>
  records.value.map((record, idx) => {
    ...
    const visitNotes =
      record.VisitNotes ?? record.visitNotes ?? record.notes ?? "";
    const date =
      record.date ?? record.Date ?? record.visitDate ?? record.VisitDate ?? "";
    const isClosed = Boolean(record.isClosed ?? record.IsClosed ?? false);
    const statusRaw = String(record.status ?? record.Status ?? "")
      .toUpperCase()
      .trim();
...
const normalizePrescriptionRows = (rows) =>
  rows.map((row, idx) => ({
    ...
    dosage:
      row.dosage ?? row.Dosage ?? row.instructions ?? row.Instructions ?? "N/A",
...
const fetchPatientHistory = async () => {
  ...
  try {
    ...
    patientHistoryRecords.value = normalizeObjectArrayResponse(
    ...
    const recordIds = patientHistoryRecords.value
      .map((record) =>
        Number(record.Id ?? record.id ?? record.recordId ?? record.RecordId),
      )
    ...
    patientHistoryPrescriptions.value = normalizePrescriptionRows(
...
const dispenseRecord = async (record) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorText || `Dispense action failed (${response.status})`,
      );
    }
...
const submitPatient = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        errorText || `Patient registration failed (${response.status})`,
      );
    }
    ...
    if (String(error?.message || "").includes("500")) {
      ...
      patientFormError.value =
        error?.message || "Unable to register patient right now.";
    }
  } finally {
    submittingPatient.value = false;
    endOutsystemsSync();
...
const initializeStripe = async () => {
  ...
  if (!stripeReady.value) {
    stripeError.value =
      "Unable to initialize Stripe Elements with the provided publishable key.";
  }
};
...
const createPaymentIntent = async () => {
  ...
  if (!Number.isFinite(recordId)) {
    throw new Error(
      `Invalid recordId from selected record: ${selectedInvoice.value.invoiceId}`,
    );
  }
  ...
  const paymentMethod =
    import.meta.env.VITE_DEFAULT_PAYMENT_METHOD || "pm_card_visa";
...
const handleConfirmAndPay = async () => {
  ...
  try {
    ...
    paymentVerifiedMessage.value =
      "Payment successful and invoice marked as Paid.";
    ...
    if (error?.response?.status === 503) {
      alert(
        "Backend Orchestrator is offline, but Stripe Library is successfully initialized.",
      );
    }
...
watch(
  ...
  (newName, oldName) => {
    if (
      oldName === "consultation-review" &&
      newName !== "consultation-review"
    ) {
      const hasPendingDraft = Boolean(
        sessionStorage.getItem(CONSULTATION_DRAFT_KEY),
      );
      if (!hasPendingDraft) {
        resetConsultationState();
      }
    ...
    <div class="mx-auto flex min-h-screen max-w-[1440px]">
      <aside class="w-72 border-r border-[#E8EAED] bg-white px-6 py-8">
        <div class="mb-10 flex items-center gap-3">
          ...
          <div>
            <p
              class="text-xs font-semibold uppercase tracking-[0.2em] text-[#5f6368]"
            >
              MedFlow
            </p>
      ...
      <main class="flex-1 px-10 py-8">
        <header class="mb-8 flex items-center justify-between">
          ...
          <div class="flex items-center gap-3">
            <span
              v-if="outsystemsSyncing"
              class="rounded-full bg-[#E8F0FE] px-3 py-1 text-xs font-semibold text-[#1a73e8]"
            >
              Syncing...
            </span>
            <button
        ...
        >
          ...
          <div
            v-else
            class="overflow-hidden rounded-xl border border-[#E8EAED]"
          >
        ...
        <section v-if="activeView === 'records'" class="space-y-5">
          ...
          <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
            ...
            <p class="mt-1 text-sm text-[#5f6368]">Submits visit notes.</p>
            ...
            <div
              class="mt-5 rounded-2xl border border-[#E8EAED] bg-[#F8F9FA] p-4"
            >
              <div
              ...
              >
                <div>
                  ...
                  <p class="text-sm text-[#5f6368]">
                    Search the catalogue and set the quantity for each drug
                    before review.
                  </p>
                </div>
              ...
              <div class="mt-4">
                ...
                <p
                  v-else-if="consultationFilteredDrugs.length === 0"
                  class="text-sm text-[#5f6368]"
                >
                  No drugs match your search.
                </p>
                <div
                  v-else
                  class="overflow-hidden rounded-xl border border-[#E8EAED] bg-white"
                >
                  <table class="min-w-full divide-y divide-[#E8EAED] text-sm">
                    ...
                    <tbody class="divide-y divide-[#F1F3F4] bg-white">
                      <tr
                        v-for="drug in consultationFilteredDrugs"
                        :key="drug.id"
                        class="hover:bg-[#F8F9FA]"
                      >
                        <td class="px-4 py-3 font-medium text-[#202124]">
                          {{ drug.name }}
                        </td>
              ...
              <div class="mt-4 flex flex-wrap gap-4 text-sm text-[#5f6368]">
                <span
                  >Selected drugs: {{ consultationSelectedDrugs.length }}</span
                >
                <span>Total units: {{ consultationSelectedDrugCount }}</span>
                <span
                  >Total value: SGD
                  {{ consultationSelectedDrugTotal.toFixed(2) }}</span
                >
              </div>
            </div>
            <button
            ...
            <p
              ...
              class="mt-3 rounded-lg bg-[#FDECEC] p-3 text-sm text-[#B3261E]"
            >
              {{ consultationError }}
            </p>
            <p
              v-if="consultationSuccess"
              class="mt-3 rounded-lg bg-[#E6F4EA] p-3 text-sm text-[#188038]"
            >
              {{ consultationSuccess }}
            </p>

            <div
              v-if="consultationNewRecord || consultationHistory.length"
              class="mt-4 rounded-lg border border-[#E8EAED] bg-[#F8F9FA] p-4"
            >
              <p
                class="text-xs font-semibold uppercase tracking-[0.12em] text-[#5f6368]"
              >
                Consultation Response
              </p>
              <p
                v-if="consultationRecordId !== null"
                class="mt-2 text-sm text-[#202124]"
              >
                Record ID: {{ consultationRecordId }}
              </p>
              <p
                v-if="consultationNewRecord"
                class="mt-2 text-sm text-[#202124]"
              >
                New Record: {{ JSON.stringify(consultationNewRecord) }}
              </p>
              <p class="mt-2 text-sm text-[#5f6368]">
                History Count: {{ consultationHistory.length }}
              </p>
            </div>
          </div>

          <p v-if="loadingRecords" class="text-sm text-[#5f6368]">
            Loading records...
          </p>
          <p
          ...
          <p
            v-else-if="dispenseError"
            class="rounded-lg bg-[#FDECEC] p-3 text-sm text-[#B3261E]"
          >
            {{ dispenseError }}
          </p>
          <p
            v-else-if="dispenseSuccess"
            class="rounded-lg bg-[#E6F4EA] p-3 text-sm text-[#188038]"
          >
            {{ dispenseSuccess }}
          </p>
          ...
          <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            >
              <p
                class="text-xs font-semibold uppercase tracking-[0.16em] text-[#5f6368]"
              >
                Record #{{ record.Id ?? idx + 1 }}
              </p>
              ...
              <p class="mt-1 text-sm text-[#5f6368]">
                {{
                  record.VisitNotes || record.diagnosis || "No notes available"
                }}
              </p>
              <p class="mt-3 text-sm text-[#5f6368]">
                Visit: {{ record.date || "N/A" }}
              </p>
              <p class="mt-1 text-sm text-[#5f6368]">
              ...
              >
                <span class="inline-flex items-center gap-2">
                  <LoaderCircle
                    v-if="dispensingRecordId === record.Id"
                    class="h-4 w-4 animate-spin"
                  />
                  {{
                    record.isClosed
              ...
              <p class="mt-2 text-xs text-[#5f6368]">
                Closing this record (<code>isClosed=true</code>) is the trigger
                that medication has been dispensed and billing is ready.
              </p>
            </article>
          </div>
        </section>
        ...
        <section v-if="activeView === 'history'" class="space-y-5">
          ...
          <div class="grid gap-5 xl:grid-cols-2">
            <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
              ...
              <p
                v-if="loadingPatientHistory"
                class="mt-3 text-sm text-[#5f6368]"
              >
                Loading records...
              </p>
              <p
                v-else-if="patientHistoryRecords.length === 0"
                class="mt-3 text-sm text-[#5f6368]"
              >
              ...
              <div
                v-else
                class="mt-4 overflow-hidden rounded-xl border border-[#E8EAED]"
              >
                <table class="min-w-full divide-y divide-[#E8EAED] text-sm">
                  <thead class="bg-[#F8F9FA]">
                    <tr class="text-left text-[#5f6368]">
                  ...
                  <tbody class="divide-y divide-[#F1F3F4] bg-white">
                    <tr
                      v-for="(row, idx) in patientHistoryRecords"
                      :key="row.Id ?? row.id ?? idx"
                    >
                      <td class="px-4 py-3">{{ row.Id ?? row.id ?? "N/A" }}</td>
                      <td class="px-4 py-3">
                        {{ row.date ?? row.Date ?? row.visitDate ?? "N/A" }}
                      </td>
                      <td class="px-4 py-3">
                        {{
            ...
            <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
              ...
              <p
                v-if="patientHistoryPrescriptionError"
                class="mt-3 rounded-lg bg-[#FDECEC] p-3 text-sm text-[#B3261E]"
              >
              ...
              <p
                v-else-if="patientHistoryPrescriptions.length === 0"
                class="mt-3 text-sm text-[#5f6368]"
              >
              ...
              >
                <table class="min-w-full divide-y divide-[#E8EAED] text-sm">
                  ...
                  <tbody class="divide-y divide-[#F1F3F4] bg-white">
                    <tr
                      v-for="row in patientHistoryPrescriptions"
                      :key="row.id"
                    >
        ...
        <section v-if="activeView === 'patients'" class="space-y-5">
          <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
            ...
            <p class="text-sm text-[#5f6368]">
              Search and view all registered patients with detailed information.
            </p>
            ...
            <div v-if="loadingPatients" class="mt-4 text-sm text-[#5f6368]">
              Loading patients...
            </div>
            <div
              v-else-if="filteredPatients.length === 0 && !patientsError"
              class="mt-4 text-sm text-[#5f6368]"
            >
              No patients found.
            </div>
            <div
        ...
        <section v-if="activeView === 'payments'" class="space-y-6">
          <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
            ...
            <p class="mt-1 text-sm text-[#5f6368]">
              Fetches closed records with patient details (JOIN-style billing
              view), then allows Mark as Paid updates.
            </p>
            <p
              v-if="billingError"
              class="mt-3 rounded-lg bg-[#FDECEC] p-3 text-sm text-[#B3261E]"
            ...
            <div v-if="loadingBilling" class="mt-4 text-sm text-[#5f6368]">
            ...
            <div
              ...
              class="mt-4 text-sm text-[#5f6368]"
            >
              No closed records found for billing.
            </div>
            <div
              v-else
              class="mt-4 overflow-hidden rounded-xl border border-[#E8EAED]"
              ...
              >
                <div>
                  <p class="text-sm font-semibold">
                    {{ invoice.invoiceId }} - {{ invoice.patientName }}
                  </p>
          ...
          >
            ...
            <p class="mt-1 text-sm text-[#5f6368]">
              Invoice {{ selectedInvoice?.invoiceId }} for
          ...
          <div
            v-if="paymentStep === 3"
            class="rounded-2xl border border-[#CEEAD6] bg-white p-6"
          >
            <p
              class="text-xs font-semibold uppercase tracking-[0.16em] text-[#188038]"
            >
              Payment Completed
            </p>
            <h4 class="mt-2 text-2xl font-semibold">Payment Received</h4>
            <p class="mt-2 text-sm text-[#5f6368]">
              Payment intent {{ paymentIntentId || "N/A" }} succeeded for
              invoice {{ selectedInvoice?.invoiceId }}.
            </p>
            <p
              v-if="paymentVerifiedMessage"
              class="mt-3 rounded-lg bg-[#E6F4EA] p-3 text-sm font-medium text-[#188038]"
            >
              {{ paymentVerifiedMessage }}
            </p>
            <button
              class="mt-5 rounded-lg bg-[#1a73e8] px-5 py-2.5 text-sm font-medium text-white hover:bg-[#1765cc]"
            >
          ...
          <div class="rounded-2xl border border-[#E8EAED] bg-white p-4">
            <h4 class="text-sm font-semibold text-[#202124]">
              Stripe Live Console Logs
            </h4>
            <div
              class="mt-3 max-h-36 overflow-auto rounded-lg border border-[#E8EAED] bg-[#F8F9FA] p-3 font-mono text-xs text-[#5f6368]"
            >
              <p v-if="paymentConsoleLogs.length === 0">
                No payment intent logs yet.
              </p>
              <p v-for="(entry, index) in paymentConsoleLogs" :key="index">
                {{ entry }}
              </p>
            </div>
          </div>
