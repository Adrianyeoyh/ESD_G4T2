...
const fetchDrugs = async () => {
  ...
  try {
    ...
    inventory.value = sortedDrugs(rows)
...
const submitConsultation = async () => {
  ...
  const prescribedDrugId = Number(consultationForm.value.prescribedDrugId)
  const prescribedQuantity = Number(consultationForm.value.prescribedQuantity)
  if (!nric || !visitNotes) {
    consultationError.value = 'NRIC and visitNotes are required.'
    return
  }
  if (!Number.isFinite(prescribedDrugId) || prescribedDrugId <= 0) {
    consultationError.value = 'Please select a prescribed drug.'
    return
  }
  if (!Number.isFinite(prescribedQuantity) || prescribedQuantity <= 0) {
    consultationError.value = 'Prescribed quantity must be at least 1.'
    return
  }
  ...
  try {
    ...
    const newRecordId = Number(
      consultationNewRecord.value?.Id ??
        consultationNewRecord.value?.id ??
        consultationNewRecord.value?.recordId ??
        0,
    )
    await applyPrescriptionToInventory(newRecordId, prescribedDrugId, prescribedQuantity)
    consultationSuccess.value = 'Consultation created and inventory updated successfully.'
    consultationForm.value.visitNotes = ''
    consultationForm.value.prescribedDrugId = ''
    consultationForm.value.prescribedQuantity = 1
    await fetchRecords()
    await loadBillingRows()
  } catch (error) {
    consultationError.value = error?.message || 'Unable to submit consultation right now.'
  } finally {
        ...
        <section v-if="activeView === 'records'" class="space-y-5">
          ...
          <div class="rounded-2xl border border-[#E8EAED] bg-white p-6">
            ...
            <p class="mt-2 text-sm text-[#5f6368]">
              Recommended Dosage:
              <span class="font-medium text-[#202124]">
                {{ selectedPrescribedDrug?.dosage || 'N/A' }}
              </span>
            </p>
            <button
